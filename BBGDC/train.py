...
from utils import load_data, accuracy_mrun_np, normalize_torch, var_mrun_np
...
for i in range(nepochs):
    ...
    var_test_tr = var_mrun_np(outs, labels_np, idx_test_np)
    
    print('Epoch: {:04d}'.format(i+1)
          , 'nll: {:.4f}'.format(nll_loss.item())
          , 'kld: {:.4f}'.format(kld_loss.item())
          , 'acc_val: {:.4f}'.format(acc_val_tr)
          , 'acc_test: {:.4f}'.format(acc_test_tr), '+- {:.3f}'.format(var_test_tr))
    print('----------------------')
    
    accuracy.append(acc_test_tr)

print("Accuracy in test: {:.1f}".format(statistics.mean(accuracy)), "+- {:.1f}".format(statistics.stdev(accuracy)) )
